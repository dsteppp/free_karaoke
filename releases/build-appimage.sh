...
# ── Критично: заменяем симлинки python на реальные копии ──────────
# venv --copies копирует библиотеки, но bin/python может остаться симлинком
# на системный python. Для AppImage это фатально — на чужой системе
# /usr/bin/python3.11 может отсутствовать.
for link in "$AMD_VENV/bin/python" "$AMD_VENV/bin/python3" "$AMD_VENV/bin/python3.11"; do
    if [ -L "$link" ]; then
...
# ── Критично: заменяем симлинки python на реальные копии ──────────
for link in "$NVIDIA_VENV/bin/python" "$NVIDIA_VENV/bin/python3" "$NVIDIA_VENV/bin/python3.11"; do
    if [ -L "$link" ]; then
        target=$(readlink -f "$link")
        rm -f "$link"
        cp "$target" "$link"
...
# ── Проверка: PyQt6-WebEngine установлен в NVIDIA venv ────────────
if ! "$NVIDIA_VENV/bin/python" -c "from PyQt6.QtWebEngineCore import QWebEngineSettings" 2>/dev/null; then
    echo "   ⚠️  PyQt6.QtWebEngineCore НЕ доступен в NVIDIA venv — переустановка..."
    source "$NVIDIA_VENV/bin/activate"
    pip install --cache-dir="$CACHE_DIR/pip" PyQt6==6.7.0 PyQt6-WebEngine==6.7.0 || {
        echo "   ❌ Не удалось установить PyQt6-WebEngine в NVIDIA venv!"
        exit 1
    }
    deactivate || true
    echo "   ✅ PyQt6-WebEngine переустановлен"
else
    echo "   ✅ PyQt6.QtWebEngineCore доступен"
fi
...
# Collect Qt6 .so files — приоритет NVIDIA venv (он используется на GPU-системах)
# Если NVIDIA venv Qt нет — fallback на AMD venv
QT_SOURCE="$NVIDIA_VENV/lib/python3.11/site-packages/PyQt6/Qt6/lib"
QT_SOURCE_LABEL="NVIDIA venv"
if [ ! -d "$QT_SOURCE" ]; then
    QT_SOURCE="$AMD_VENV/lib/python3.11/site-packages/PyQt6/Qt6/lib"
    QT_SOURCE_LABEL="AMD venv (fallback)"
fi
if [ -d "$QT_SOURCE" ]; then
    echo "   Copying Qt6 libraries from PyQt6 wheel ($QT_SOURCE_LABEL)..."
    cp -L "$QT_SOURCE"/*.so* "$SYS_LIBS_DIR/" 2>/dev/null || true
    QT_SIZE=$(du -sh "$SYS_LIBS_DIR" | cut -f1)
    echo "   ✅ Qt6 libraries bundled ($QT_SIZE)"
else
    echo "   ⚠️  Qt6 not found in either venv — may fail at runtime"
fi

# Also copy Qt6 plugins and QML — приоритет NVIDIA venv
QT_PLUGINS_SRC="$NVIDIA_VENV/lib/python3.11/site-packages/PyQt6/Qt6/plugins"
...
# ── Python isolation ────────────────────────────────────────────────
# Запрещаем Python искать пакеты в ~/.local и системных site-packages.
# Все зависимости — ТОЛЬКО внутри venv AppImage.
export PYTHONNOUSERSITE=1
# Очищаем PYTHONPATH чтобы Python не заглядывал в системные пути.
unset PYTHONPATH
# Сбрасываем PYTHONHOME — venv сам настроит правильный sys.prefix.
unset PYTHONHOME
