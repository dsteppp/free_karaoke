...
# Определяем GPU с учётом гибридных конфигураций
GPU_TYPE="CPU"
GPU_DETAILS=""
HAS_HYBRID_GPU=false

if command -v lspci &> /dev/null; then
    HAS_NVIDIA=$(lspci | grep -iE "nvidia" | wc -l)
    HAS_AMD=$(lspci | grep -iE "radeon|amd.*graphics|advanced micro devices" | wc -l)
    
    if [ "$HAS_NVIDIA" -gt 0 ] && [ "$HAS_AMD" -gt 0 ]; then
        HAS_HYBRID_GPU=true
        GPU_TYPE="HYBRID"
        GPU_DETAILS="AMD+NVIDIA (hybrid)"
        log_warn "Обнаружена гибридная графика: $GPU_DETAILS"
        ...
        log_info "Дискретный GPU не обнаружен — будет использоваться CPU/интегрированная графика"
    fi
fi
...
# Списки пакетов — с корректными именами для всех дистров
APT_PACKAGES="python3-venv python3-pip curl git ffmpeg libsndfile1 portaudio19-dev yad python3-gi gir1.2-gtk-3.0 qt6-wayland"
DNF_PACKAGES="python3-devel python3-virtualenv curl git ffmpeg libsndfile portaudio-devel yad python3-gobject gtk3 qt6-qtwayland"
PACMAN_PACKAGES="python-virtualenv python-pip curl git ffmpeg libsndfile portaudio yad python-gobject gtk3 qt6-wayland"
ZYPPER_PACKAGES="python3-devel python3-virtualenv curl git ffmpeg libsndfile1 portaudio-devel yad python3-gobject gtk3 qt6-wayland"
...
# =============================================================================
# ПАТЧ DIFFQ (Обход C++ компиляции для SteamOS и систем без gcc/clang)
# =============================================================================
if ! uv pip show diffq &>/dev/null; then
    log_info "Применение патча diffq (Pure-Python режим для SteamOS)..."
    rm -f diffq.tar.gz
    rm -rf .t_diffq

    # Скачиваем исходники
    curl -fSL -o diffq.tar.gz "https://files.pythonhosted.org/packages/source/d/diffq/diffq-0.2.4.tar.gz"
    mkdir -p .t_diffq
    tar -xzf diffq.tar.gz -C .t_diffq

    # Переписываем setup.py, чтобы он устанавливал только Python файлы, игнорируя C-расширения
    cat > ".t_diffq/diffq-0.2.4/setup.py" << 'EOF'
from setuptools import setup, find_packages
setup(name='diffq', version='0.2.4', packages=find_packages())
EOF

    # Устанавливаем пропатченную версию
    if uv pip install ./.t_diffq/diffq-0.2.4; then
        log_success "Патч diffq успешно применен"
    else
        log_error "Сбой применения патча diffq!"
        exit 1
    fi

    # Уборка мусора
    rm -rf diffq.tar.gz .t_diffq
fi
# =============================================================================
...
if ! uv pip install --prerelease=allow --index-strategy unsafe-best-match -r "$INSTALL_DIR/core/requirements.txt"; then
...
echo "      • App Name: Free Karaoke (или любое)"
echo "      • Icon URL: можно оставить значение по умолчанию"
echo "      • App Website URL: ОБЯЗАТЕЛЬНО начинается с https:// (например https://genius.com)"
echo "      • Redirect URI: тоже нужно заполнить (например www.google.com)"
echo "   4. Нажмите 'Save'"
echo "   5. На странице приложения нажмите 'Generate Access Token'"
echo "   6. Скопируйте появившийся 'Client Access Token'"
echo ""
...
# ─────────────────────────────────────────────────────────────────────────────
# 13. Создание run.sh скрипта (универсальная обёртка с авто-совместимостью)
# ─────────────────────────────────────────────────────────────────────────────
...
#!/bin/bash
# =============================================================================
# Free Karaoke — Универсальный скрипт запуска
# Автоматически подстраивается под графическую конфигурацию системы
# =============================================================================
...
# -----------------------------------------------------------------------------
# 1. Загрузка переменных окружения из .env файлов
# -----------------------------------------------------------------------------
for envfile in "$CORE_DIR/.env" "$CORE_DIR/.env.cache"; do
    if [[ -f "$envfile" ]]; then
...
# -----------------------------------------------------------------------------
# 2. Авто-определение графической конфигурации
# -----------------------------------------------------------------------------
DETECT_WAYLAND=false
[[ "$XDG_SESSION_TYPE" == "wayland" || -n "$WAYLAND_DISPLAY" ]] && DETECT_WAYLAND=true

DETECT_HYBRID=false
if command -v lspci &>/dev/null; then
    N=$(lspci 2>/dev/null | grep -ciE 'nvidia' || true)
    N=${N:-0}
    A=$(lspci 2>/dev/null | grep -ciE 'radeon|amd.*graphics' || true)
    A=${A:-0}
    [[ $N -gt 0 && $A -gt 0 ]] && DETECT_HYBRID=true
fi

# -----------------------------------------------------------------------------
# 3. Графические флаги — приоритет: СТАБИЛЬНОСТЬ > ПРОИЗВОДИТЕЛЬНОСТЬ
# -----------------------------------------------------------------------------

# === Базовые флаги для ВСЕХ конфигураций ===
export QTWEBENGINE_CHROMIUM_FLAGS="--disable-dev-shm-usage"

# === Конфигурация для гибридной графики (самая проблемная) ===
if [[ "$DETECT_HYBRID" == "true" ]]; then
    # Отключаем GPU в QtWebEngine — стабильнее на 99% систем
    export QTWEBENGINE_CHROMIUM_FLAGS="$QTWEBENGINE_CHROMIUM_FLAGS --disable-gpu --disable-software-rasterizer"
    # Используем X11-бэкенд (Wayland + hybrid = нестабильно)
    export QT_QPA_PLATFORM=xcb
    # Очищаем конфликтные переменные NVIDIA Prime
    unset __GLX_VENDOR_LIBRARY_NAME __NV_PRIME_RENDER_OFFLOAD __VK_LAYER_NV_optimus 2>/dev/null
    # Явно предпочитаем AMD для Mesa-приложений
    export MESA_D3D12_DEFAULT_ADAPTER_NAME=AMD

# === Конфигурация для NVIDIA (дискретная) ===
elif [[ "$DETECT_HYBRID" == "false" ]] && lspci 2>/dev/null | grep -qiE 'nvidia'; then
    if [[ "$DETECT_WAYLAND" == "true" ]]; then
        # NVIDIA + Wayland: используем X11 для стабильности
        export QT_QPA_PLATFORM=xcb
        export QTWEBENGINE_CHROMIUM_FLAGS="$QTWEBENGINE_CHROMIUM_FLAGS --use-gl=egl --disable-gpu-sandbox"
    else
        # NVIDIA + X11: можно попробовать аппаратное ускорение
        export QT_QPA_PLATFORM=xcb
        export QTWEBENGINE_CHROMIUM_FLAGS="$QTWEBENGINE_CHROMIUM_FLAGS --use-gl=desktop --disable-gpu-sandbox"
    fi

# === Конфигурация для AMD (дискретная или интегрированная) ===
elif lspci 2>/dev/null | grep -qiE 'radeon|amd.*graphics'; then
    if [[ "$DETECT_WAYLAND" == "true" ]]; then
        # AMD + Wayland: пробуем Wayland, но с флагами стабильности
        export QT_QPA_PLATFORM=wayland
        export QTWEBENGINE_CHROMIUM_FLAGS="$QTWEBENGINE_CHROMIUM_FLAGS --use-gl=angle --angle=gl --disable-gpu-sandbox"
        # Для старых драйверов AMD на Manjaro/Arch
        export HSA_OVERRIDE_GFX_VERSION=11.0.0
    else
        # AMD + X11: стабильный режим
        export QT_QPA_PLATFORM=xcb
        export QTWEBENGINE_CHROMIUM_FLAGS="$QTWEBENGINE_CHROMIUM_FLAGS --use-gl=desktop"
    fi

# === Fallback для неизвестных конфигураций / CPU ===
else
    # Максимально совместимый режим: отключаем GPU, используем X11
    export QTWEBENGINE_CHROMIUM_FLAGS="$QTWEBENGINE_CHROMIUM_FLAGS --disable-gpu --disable-software-rasterizer"
    export QT_QPA_PLATFORM=xcb
fi

# -----------------------------------------------------------------------------
# 4. Настройки масштабирования и стабильности
# -----------------------------------------------------------------------------
# Включаем автоматическое HiDPI-масштабирование Qt
export QT_ENABLE_HIGHDPI_SCALING=1
export QT_AUTO_SCREEN_SCALE_FACTOR=1

# Для Wayland: дополнительные настройки
if [[ "$DETECT_WAYLAND" == "true" ]]; then
    export QT_WAYLAND_ENABLE_WINDOW_DECORATION=1
    export QT_WAYLAND_RECONNECT=1
    # На случай проблем с масштабированием в Wayland
...
# -----------------------------------------------------------------------------
# Активируем venv если он есть в родительской директории
if [[ -f "$DIR/.venv/bin/activate" ]]; then
    source "$DIR/.venv/bin/activate" 2>/dev/null || true
fi

# Запускаем приложение
exec "$CORE_DIR/run.sh" "$@"
RUNSCRIPT

chmod +x "$INSTALL_DIR/run.sh"
log_success "Универсальный скрипт запуска создан: $INSTALL_DIR/run.sh"
echo ""
...
echo ""
echo "💡 Советы по запуску:"
echo "   • Если интерфейс не отрисовывается, попробуйте:"
echo "     QT_QPA_PLATFORM=xcb $INSTALL_DIR/run.sh"
echo "   • Для включения экспериментального GPU-ускорения:"
echo "     Откройте $INSTALL_DIR/run.sh и закомментируйте строку с --disable-gpu"
echo ""
echo "Нажмите Enter для закрытия окна..."
read -r
