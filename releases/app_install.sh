...
# ─────────────────────────────────────────────────────────────────────────────
# 8. Модификация launcher.py: отключаем XDG_CACHE_HOME (ломает QtWebEngine)
# ─────────────────────────────────────────────────────────────────────────────
log_step "Корректировка launcher.py для совместимости с QtWebEngine"
echo ""

# Отключаем XDG_CACHE_HOME в launcher.py — это вызывает OzoneImageBacking errors
# Используем расширенный regex для обработки любых пробельных символов (пробелы/табы)
if [ -f "$INSTALL_DIR/core/launcher.py" ]; then
    sed -i 's/^os\.environ\.setdefault("XDG_CACHE_HOME"/# os.environ.setdefault("XDG_CACHE_HOME"/' "$INSTALL_DIR/core/launcher.py"
    log_success "XDG_CACHE_HOME отключён в launcher.py"
else
    log_warn "launcher.py не найден, пропускаем корректировку"
fi

echo ""

# 9. Генерация requirements.txt под конфигурацию
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 10. Создание виртуального окружения и установка пакетов
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 13. Создание .env.cache для изоляции кэшей
# ─────────────────────────────────────────────────────────────────────────────
log_step "Настройка изоляции кэшей"
echo ""

# Не устанавливаем XDG_CACHE_HOME — это ломает QtWebEngine (OzoneImageBacking errors)
# Используем только специфичные переменные для ML-кэшей
...
# ─────────────────────────────────────────────────────────────────────────────
# 14. Создание run.sh скрипта (обертка)
# ─────────────────────────────────────────────────────────────────────────────
