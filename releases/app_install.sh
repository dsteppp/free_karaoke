...
# ─────────────────────────────────────────────────────────────────────────────
# 7. Копирование файлов программы из репозитория
# ─────────────────────────────────────────────────────────────────────────────
log_step "Копирование файлов программы"
echo ""

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CORE_SOURCE="$SCRIPT_DIR/../core"

# Проверяем, есть ли исходная папка core рядом со скриптом
if [ -d "$CORE_SOURCE" ] && [ -f "$CORE_SOURCE/main.py" ]; then
    log_info "Копируем файлы из локальной папки core..."
    
    # Копируем все файлы включая скрытые
    cp -av "$CORE_SOURCE/." "$INSTALL_DIR/core/" || {
        log_error "Не удалось скопировать файлы из $CORE_SOURCE"
        exit 1
    }
    
    log_success "Файлы скопированы из локального репозитория"
    ...
    TMP_CLONE="$INSTALL_DIR/.tmp_clone_$$"
    
    if git clone --depth 1 "$REPO_URL" "$TMP_CLONE" < /dev/null 2>&1; then
        # Копируем только содержимое папки core включая скрытые файлы
        if [ -d "$TMP_CLONE/core" ]; then
            cp -av "$TMP_CLONE/core/." "$INSTALL_DIR/core/" || {
                log_error "Не удалось скопировать файлы из клонированного репозитория"
                rm -rf "$TMP_CLONE"
                exit 1
            }
            log_success "Файлы загружены из репозитория"
        else
            log_error "В репозитории не найдена папка core"
            rm -rf "$TMP_CLONE"
            exit 1
        fi
        rm -rf "$TMP_CLONE"
    ...
    log_error "Не найдено ни локальной папки core, ни git. Невозможно продолжить установку."
    exit 1
fi

# Проверяем результат
if [ ! -f "$INSTALL_DIR/core/main.py" ]; then
    log_error "Файл main.py не найден после копирования"
    exit 1
fi
...
# ─────────────────────────────────────────────────────────────────────────────
# 12. Создание .env.cache для изоляции кэшей и отключения GPU в QtWebEngine
# ─────────────────────────────────────────────────────────────────────────────
log_step "Настройка изоляции кэшей и QtWebEngine"
echo ""
...
# Отключение GPU-ускорения в QtWebEngine для стабильности интерфейса
QTWEBENGINE_CHROMIUM_FLAGS="--no-sandbox --disable-gpu --disable-gpu-compositing --disable-software-rasterizer --disable-dev-shm-usage --disable-http-cache"
# Принудительное использование XCB для стабильности на Linux
QT_QPA_PLATFORM=xcb
# Переменные для отладки QtWebEngine (полное логирование проблем интерфейса)
QT_LOGGING_RULES="qt.webenginecontext.debug=true;qt.qpa.xcb.error=true;qt.qpa.xcb.generic=true"
QTWEBENGINEPROCESS_DEBUG=1
EOF
...
log_success "Изоляция кэшей настроена, QtWebEngine оптимизирован"
echo ""

# ─────────────────────────────────────────────────────────────────────────────
# 13. Создание run.sh скрипта (обертка) с правильной загрузкой .env и .env.cache
# ─────────────────────────────────────────────────────────────────────────────
...
#!/bin/bash
# Обертка для запуска Free Karaoke с полным логированием проблем QtWebEngine
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
cd "$DIR/core"

# Создаём директорию для логов
mkdir -p "$DIR/core/debug_logs"

# Файл лога с временной меткой
LOG_FILE="$DIR/core/debug_logs/qtwebengine_$(date +%Y%m%d_%H%M%S).log"

# Загружаем .env.cache ПЕРЕД запуском core/run.sh — это критично для QtWebEngine
...
# Экспортируем дополнительные переменные для отладки Qt
export QT_LOGGING_TO_CONSOLE=1
export QT_DEBUG_PLUGINS=1
export QT_LOGGING_RULES="qt.webenginecontext.debug=true;qt.qpa.xcb.error=true;qt.qpa.xcb.generic=true;*.debug=true"

echo "═══════════════════════════════════════════════════════" | tee "$LOG_FILE"
echo "Free Karaoke — Лог запуска $(date)" | tee "$LOG_FILE"
echo "Путь установки: $DIR" | tee "$LOG_FILE"
echo "═══════════════════════════════════════════════════════" | tee "$LOG_FILE"
echo "" | tee "$LOG_FILE"

echo "[INFO] Переменные окружения:" | tee "$LOG_FILE"
echo "  QT_QPA_PLATFORM=$QT_QPA_PLATFORM" | tee "$LOG_FILE"
echo "  QTWEBENGINE_CHROMIUM_FLAGS=$QTWEBENGINE_CHROMIUM_FLAGS" | tee "$LOG_FILE"
echo "  QT_LOGGING_RULES=$QT_LOGGING_RULES" | tee "$LOG_FILE"
echo "  QTWEBENGINEPROCESS_DEBUG=$QTWEBENGINEPROCESS_DEBUG" | tee "$LOG_FILE"
echo "" | tee "$LOG_FILE"

echo "[INFO] Запуск приложения... Ожидание вывода ошибок QtWebEngine:" | tee "$LOG_FILE"
echo "" | tee "$LOG_FILE"

# Запускаем приложение и перенаправляем весь вывод в лог
exec "$DIR/core/run.sh" "$@" 2>&1 | tee -a "$LOG_FILE"
RUNSCRIPT
...
echo "   • Логи QtWebEngine находятся в: $INSTALL_DIR/core/debug_logs/"
echo "     (файлы с именем qtwebengine_YYYYMMDD_HHMMSS.log)"
echo "   • Библиотека треков: $INSTALL_DIR/core/library (создается при запуске)"
echo ""
log_info "При проблемах с интерфейсом отправьте лог из debug_logs разработчику"
echo ""
