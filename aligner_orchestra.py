...
def propose_loom(words: list, s_idx: int, e_idx: int, t_start: float, t_end: float, strong_vad: list, weak_vad: list, empirical_data: dict = None) -> Proposal:
    ...
    if s_idx == 0:
        if strong_vad:
            ...
            if first_vocal > t_start:
                actual_start = max(t_start, first_vocal - 0.2)
    ...
    def _map_time(t_vad: float, vads: list) -> float:
        ...
        for (vs, ve) in vads:
            d = ve - vs
            if t_vad <= accum + d: return vs + (t_vad - accum)
            accum += d
        return vads[-1][1]
    ...
    for l_idx, line in enumerate(lines):
        ...
        if not line_vads: line_vads = [(l_real_s, l_real_e)]
...
def the_supreme_judge(proposals: list, words: list, s_idx: int, e_idx: int, strong_vad: list, weak_vad: list, curtains: list, empirical_data: dict = None) -> Proposal:
    ...
    for prop in proposals:
        if prop is None or len(prop.timings) != (e_idx - s_idx + 1): continue
        ...
        for i, t in enumerate(prop.timings):
            ...
            if dur < 0.08: score -= 1000.0
            elif dur < min_dur: score -= 200.0 * (min_dur - dur)
                
            overlap_curtain = calculate_overlap(t["start"], t["end"], curtains)
            if overlap_curtain > 0.05: score -= 2000.0 * overlap_curtain
        ...
        for line in lines:
            ...
            else: l_min = sum(get_phonetic_bounds(clean_texts[k], line_breaks[k])[0] for k in line)
            
            if l_dur < l_min:
                hit_the_wall = any(abs(c_s - l_e) < 0.5 for c_s, c_e in curtains)
                if not hit_the_wall: score -= 500.0 * (l_min - l_dur) 
    ...
    # 🛑 ПРАВО ВЕТО (РЕШАЮЩИЙ ФИКС ДЛЯ АРЕНЫ)
    if best_score < -500.0:
        log.warning(f"   ⚠️ Все кандидаты грубо нарушили правила (лучший балл: {best_score:.1f}). Арена забракована.")
        return None
...
def diagnostic_compass(words: list, s_idx: int, e_idx: int, audio_data: np.ndarray, t_start: float, t_end: float, model, lang: str) -> int:
    pass # Функция-заглушка для обратной совместимости, если не используется активно