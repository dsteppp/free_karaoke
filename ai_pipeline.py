...
# Твой логгер, который используется по всему проекту
...
def get_audio_metadata(file_path: str, fallback_name: str = "") -> tuple[str, str]:
    """
    Извлекает артиста и название из MP3-тегов (используется и здесь, и в main.py).
    Если тегов нет, пытается распарсить имя файла (например "Artist - Title.mp3").
    """
    ...
    # Если теги пустые, парсим имя файла
    if not artist or not title or title == fallback_name:
        fname = fallback_name if fallback_name else os.path.basename(file_path)
...
class AIPipeline:
    ...
    def separate_audio(self, audio_path: str, output_dir: str, base_name: str):
        """
        Разделение аудио. Сохраняет файлы в формате, который ждет фронтенд:
        base_name_(Vocals).mp3 и base_name_(Instrumental).mp3
        """
    ...
    def fetch_lyrics(self, artist: str, title: str, output_path: str) -> str:
        """
        Поиск текста в Genius и сохранение в _(Genius Lyrics).txt
        """
        ...
        if song and song.lyrics:
            ...
            # Сохраняем оригинальный текст для кэша и UI
    ...
    def run_pipeline(self, track_id: int, audio_path: str, artist: str, title: str, output_dir: str, base_name: str):
        ...
        # 0. Если артист или название пусты, пытаемся вытащить их из файла
        if not artist or not title:
            ext_artist, ext_title = get_audio_metadata(audio_path, base_name)
        ...
        # 4. Создаем мета-файл (нужен для scan_library из main.py)
        ...
        # Отдаем в tasks.py все необходимые пути
        return {
            "vocal": vocal_path,
